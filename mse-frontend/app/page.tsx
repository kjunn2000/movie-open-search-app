...
import { getMovieAction } from "./actions/movieAction";
import { MovieListResp } from "./types/movie";


export default function Home(context: any) {

  const { pending }  = useFormStatus()
  const [movieList, setMovieList] = useState<MovieListResp>()
  ...
  async function getMovieFromApi(formData: FormData) {
    const fetchedMovies = await getMovieAction(formData.get("movieName"));
    setMovieList(fetchedMovies);
  }

  return (
    <div>
      <form className="max-w-md mx-auto p-5" action={getMovieFromApi}>
